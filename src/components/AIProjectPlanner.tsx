            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                {!roadmap ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="relative group">
                                <textarea
                                    ...
                                    className="w-full h-48 p-6 rounded-3xl border border-slate-200 focus:ring-2 focus:ring-nexus-blue focus:border-transparent resize-none text-slate-700 bg-slate-50 focus:bg-white transition-all outline-none text-sm leading-relaxed"
                                    disabled={loading}
                                />
                                <div className="absolute bottom-4 right-4 text-[0.6rem] text-slate-300 font-bold uppercase tracking-widest">
                                    AI Context Engine
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-5 bg-slate-900 text-white rounded-2xl font-bold text-sm uppercase tracking-[0.2em] hover:bg-slate-800 disabled:opacity-50 flex items-center justify-center gap-3 transition-all shadow-xl shadow-slate-200 active:scale-[0.98]"
                            >
                                {loading ? (
                                    <>
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                                ...
                                {roadmap.steps.map((step, idx) => (
                                    <div key={idx} className="relative pl-12 pb-12 last:pb-0 group">
                                        {idx !== roadmap.steps.length - 1 && (
                                            <div className="absolute left-[7px] top-6 w-[2px] h-full bg-slate-100 group-hover:bg-nexus-blue/20 transition-colors"></div>
                                        )}

                                        <div className="absolute left-0 top-0 w-4 h-4 rounded-full border-2 border-slate-200 bg-white group-hover:border-nexus-blue transition-colors flex items-center justify-center">
                                            <div className="w-1.5 h-1.5 rounded-full bg-transparent group-hover:bg-nexus-blue transition-all"></div>
                                        </div>

                                        <div className="bg-white p-2 rounded-3xl -mt-2 transition-all group-hover:translate-x-1">
                        ...
                        <div className="pt-12 border-t border-slate-100 flex flex-col sm:flex-row gap-4 sticky bottom-0 bg-white pb-4">
                            <button
                                onClick={() => setRoadmap(null)}
                                className="flex-1 py-5 border border-slate-200 rounded-2xl text-xs font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors active:scale-95"
                            >
                                Start Over
                            </button>
                            <button
                                className="flex-1 py-5 bg-slate-900 text-white rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 active:scale-95 flex items-center justify-center gap-3"
                                onClick={handleClose}
                            >
                                Contact Architect
